...
<details>
<summary><b>New to these algorithms? Click for a one-line guide to each.</b></summary>

- **Logistic Regression** — draws a straight dividing line between the two classes and outputs a probability. Simple and explainable, but basic.
- **Gaussian Naive Bayes** — uses probability ("given these details, how likely is each outcome?"); assumes features are independent, so quick but rough.
- **K-Nearest Neighbours** — classifies a customer by majority vote of the most similar past customers.
- **Decision Tree** — a flowchart of yes/no questions leading to a verdict; easy to read but shaky alone.
- **Random Forest** — hundreds of decision trees on random data slices, voting together for a reliable answer.
- **Extra Trees** — like Random Forest but with more randomness in its splits; faster and often robust.
- **AdaBoost** — builds trees in sequence, each focusing on the cases the previous ones got wrong.
- **Gradient Boosting** — sequential trees, each reducing the leftover error of the model so far. *(Winner here.)*
- **XGBoost** — a fast, optimized gradient boosting with built-in overfitting safeguards; the one tuned for imbalance.
- **SVM (RBF)** — draws the widest-gap boundary between the classes; the RBF part lets it curve.
- **Neural Network (MLP)** — a small network of layered "neurons" that learns complex patterns. *(2nd place here.)*
